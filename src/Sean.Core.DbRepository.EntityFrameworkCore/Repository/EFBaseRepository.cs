...
namespace Sean.Core.DbRepository.EntityFrameworkCore;
...
public abstract class EFBaseRepository<TEntity> : IEFBaseRepository<TEntity> where TEntity : class
{
    ...
    {
        return _db.Database.SqlQueryRaw<T>(sql, parameters).ToList();
    }
    ...
    {
        return await _db.Database.SqlQueryRaw<T>(sql, parameters).ToListAsync();
    }
